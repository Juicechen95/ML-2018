...
class DataLoader:
    ...
    def __init__(self):
        train_data = self.load_file('../../data/titanic/train.csv')
        test_data = self.load_file('../../data/titanic/test.csv')
        ...
        for nf in self.NUM_FEAT:
            ind = self.num2cat(list(train_feat[:, nf]) + list(test_feat[:, nf]))
    ...
    def load_file(self, file_name):
        ...
        with open(file_name, 'r') as fin:
            ...
            for line in fin:
                line = line.strip()
                data.append(self.parse_feat(line))
...
def viz(feat_names, train_data, valid_data, max_depth=4):
    train_label, train_feat = train_data[0], train_data[1].toarray()
    valid_label, valid_feat = valid_data[0], valid_data[1].toarray()
    ...
    tree.export_graphviz(clf,
                        out_file=dot_data,
                        feature_names=sorted(feat_names),
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == 'task1':
        DT = DecisionTree(len(DL.feat_map), max_depth=4, verbose=False)
        DT.fit(*DL.train_data)

        print(DT.acc(*DL.train_data))
        print(DT.acc(*DL.valid_data))
    elif sys.argv[1] == 'task2':
        viz(DL.feat_map.keys(),  DL.train_data, DL.valid_data, max_depth=4)
